...
import com.type404.backend.domain.store.dto.request.MenuDTO;
import com.type404.backend.domain.store.dto.request.OpeningHourDTO;
import com.type404.backend.domain.store.dto.request.SeatDTO;
...
public class StoreService {
    ...
    private void saveStoreSeat(SeatDTO seatDto, StoreInfoEntity store) {
        if (seatDto == null) {
    ...
    private void saveStoreOpeningHours(List<OpeningHourDTO> hours, StoreInfoEntity store) {
        if (hours == null || hours.isEmpty()) {
            throw new CustomException(ErrorCode.INVALID_PARAMETER, "영업 시간 정보가 최소 하나 이상 필요합니다.");
        }

        for (OpeningHourDTO oh : hours) {
    ...
    private void saveStoreMenus(List<MenuDTO> menus, List<MultipartFile> menuImgs, StoreInfoEntity store) {
